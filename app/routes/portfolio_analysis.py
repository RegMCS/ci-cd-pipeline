...
async def get_cumulative_price(
    portfolioId: str = Query(..., description="Portfolio identifier"),
    startDate: date = Query(..., description="Start date for the cumulative price series"),
...
async def get_correlation(
    ...
    try:
        ...
        if portfolioId1 == portfolioId2:
            raise HTTPException(status_code=400, detail="Portfolio IDs must be different")
        
        return portfolio_service.get_correlation(portfolioId1, portfolioId2, startDate, endDate)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except HTTPException:
        raise
...
async def get_tracking_error(
    ...
    startDate: date = Query(..., description="Start date for tracking error calculation"),
    ...
    try:
        ...
        return portfolio_service.get_tracking_error(portfolioId, benchmarkId, startDate, endDate)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except HTTPException:
        raise
